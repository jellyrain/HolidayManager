...
const { schedulingUpdate, overTimeLogAdd } = window.api as {
...
const model: scheduling & overTimeLog = reactive({
  id: uuidv4(),
...
const handleAdd = () => {
  ...
  if (scheduling && scheduling.vacationTime != null) {
    store.scheduling[index].vacationTime =
      store.scheduling[index].vacationTime + store.getOverTimeDateById(model.overTimeId)
